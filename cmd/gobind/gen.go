...
func genPkg(lang string, p *types.Package, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		buf.Reset()
		w, closer = writer(filepath.Join("src", "gomobile", pname+"_android.c"))
		processErr(g.GenC())
		io.Copy(w, &buf)
		closer()
		buf.Reset()
		w, closer = writer(filepath.Join("src", "gomobile", pname+"_android.h"))
		processErr(g.GenH())
		io.Copy(w, &buf)
		closer()
		...
		if p == nil {
			...
			copyFile(filepath.Join("src", "gomobile", "seq_android.c"), filepath.Join(javaPkg.Dir, "seq_android.c.support"))
			copyFile(filepath.Join("src", "gomobile", "seq_android.go"), filepath.Join(javaPkg.Dir, "seq_android.go.support"))
			copyFile(filepath.Join("src", "gomobile", "seq_android.h"), filepath.Join(javaPkg.Dir, "seq_android.h"))
		}
	case "go":
		w, closer := writer(filepath.Join("src", "gomobile", fname))
		conf.Writer = w
		processErr(bind.GenGo(conf))
		closer()
		var buf bytes.Buffer
		w, closer = writer(filepath.Join("src", "gomobile", pname+".h"))
		genPkgH(w, pname)
		io.Copy(w, &buf)
		closer()
		w, closer = writer(filepath.Join("src", "gomobile", "seq.h"))
		genPkgH(w, "seq")
		io.Copy(w, &buf)
		closer()
		...
		copyFile(filepath.Join("src", "gomobile", "seq.go"), filepath.Join(bindPkg.Dir, "seq.go.support"))
	case "objc":
		var buf bytes.Buffer
		g := &bind.ObjcGen{
			Generator: &bind.Generator{
		...
		g.Init(otypes)
		w, closer := writer(filepath.Join("src", "gomobile", pname+"_darwin.h"))
		processErr(g.GenGoH())
		io.Copy(w, &buf)
		closer()
		hname := strings.Title(fname[:len(fname)-2]) + ".objc.h"
		w, closer = writer(filepath.Join("src", "gomobile", hname))
		processErr(g.GenH())
		io.Copy(w, &buf)
		closer()
		mname := strings.Title(fname[:len(fname)-2]) + "_darwin.m"
		w, closer = writer(filepath.Join("src", "gomobile", mname))
		conf.Writer = w
		processErr(g.GenM())
		io.Copy(w, &buf)
		closer()
		if p == nil {
			...
			copyFile(filepath.Join("src", "gomobile", "seq_darwin.m"), filepath.Join(objcPkg.Dir, "seq_darwin.m.support"))
			copyFile(filepath.Join("src", "gomobile", "seq_darwin.go"), filepath.Join(objcPkg.Dir, "seq_darwin.go.support"))
			copyFile(filepath.Join("src", "gomobile", "ref.h"), filepath.Join(objcPkg.Dir, "ref.h"))
			copyFile(filepath.Join("src", "gomobile", "seq_darwin.h"), filepath.Join(objcPkg.Dir, "seq_darwin.h"))
		}
	default:
...
#ifdef __GOMOBILE_ANDROID__
#include "%[1]s_android.h"
#endif
#ifdef __GOMOBILE_DARWIN__
#include "%[1]s_darwin.h"
#endif`, pname)
}

func genObjcPackages(ctx *build.Context, dir string, types []*objc.Named, embedders []importers.Struct) error {
	srcDir := *outdir
	if srcDir == "" {
		srcDir = dir
	}
	...
	for i, opkg := range cg.Packages() {
		pkgDir := filepath.Join(srcDir, "src", "ObjC", opkg)
	...
	buf.Reset()
	cg.GenInterfaces()
	objcBase := filepath.Join(srcDir, "src", "ObjC")
	...
	goBase := filepath.Join(srcDir, "src", "gomobile")
	...
	cmd.Env = append(os.Environ(), "GOPATH="+srcDir)
	cmd.Env = append(cmd.Env, "GOROOT="+ctx.GOROOT)
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to go install the generated ObjC wrappers: %v: %s", err, out)
	}
	return nil
}
...
func genJavaPackages(ctx *build.Context, dir string, classes []*java.Class, embedders []importers.Struct) error {
	...
	for i, jpkg := range cg.Packages() {
		pkgDir := filepath.Join(srcDir, "src", "Java", jpkg)
		if err := os.MkdirAll(pkgDir, 0700); err != nil {
			return err
		}
		pkgFile := filepath.Join(pkgDir, "package.go")
	...
	buf.Reset()
	cg.GenInterfaces()
	javaBase := filepath.Join(srcDir, "src", "Java")
	...
	cmd.Env = append(cmd.Env, "GOROOT="+ctx.GOROOT)
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to go install the generated Java wrappers: %v: %s", err, out)
	}
	return nil
}
